...
import { appendEnrichment, enrichments, enrichmentDate } from './guide-enrichment.mjs';
...
const { data: documents, error } = await client.from('travel_documents').select('*').in('activity_id', Object.keys(enrichments));
if (error) throw error;
const missing = Object.keys(enrichments).filter((id) => !documents.some((d) => d.activity_id === id));
if (missing.length) throw new Error(`ガイドが見つかりません: ${missing.join(', ')}`);
const updates = documents.map((doc) => ({ before: doc, after: appendEnrichment(doc, enrichments[doc.activity_id]) }))
  .filter(({ before, after }) => JSON.stringify(before.blocks) !== JSON.stringify(after.blocks));
console.log(`追記対象: ${updates.length}件 / 調査済み${documents.length}件。`);
for (const { before } of updates) console.log(`・${before.title}`);
if (!process.argv.includes('--write')) process.exit(0);
const backup = join(tmpdir(), `trip-guide-backup-${Date.now()}.json`);
...
for (const { before, after } of updates) {
  const { data: saved, error: updateError } = await client.from('travel_documents')
    .update({ blocks: after.blocks, checked_at: enrichmentDate, revision: before.revision + 1, updated_at: new Date().toISOString() })
...
console.log(`追記・保存検証完了: ${updates.length}件。旅程revision ${workspace.revision} → ${afterWorkspace.revision}。`);
