...
export default function PlaceSearch({ value, onChange, onSelect, apiKey, onRequestKey, variant = 'field' }) {
  ...
  const search = async () => {
    ...
    if (!apiKey || !window.google?.maps) { setStatus('error'); onRequestKey?.(); return; }
    setCandidates([]);
    setStatus('loading');
    try {
  ...
  useEffect(() => {
    if (value === selectedValue.current || value.trim().length < 2) return;
  ...
  return <div className={variant === 'map' ? 'map-search place-search' : 'place-search'}>
    ...
    {status !== 'idle' && <div className="place-candidates" id={listId}>
      ...
      </> : <p role={status === 'error' ? 'alert' : 'status'}>{status === 'loading' ? '検索中…' : status === 'empty' ? '場所が見つかりません。検索語を変えてください。' : '検索できませんでした。接続を確認して再試行してください。'}</p>}
    </div>}
  </div>;
}
