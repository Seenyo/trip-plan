...
export function swapDomesticActivities(trip) {
  ...
  if (!september20 || !september21) return trip;
  const everyActivity = [...september20.activities, ...september21.activities];
  const allMovedIds = [...TO_SEPTEMBER_21, ...TO_SEPTEMBER_20];
  // Older bundled itineraries did not contain these user-added stops and cannot be inferred.
  if (!allMovedIds.every((id) => everyActivity.some((activity) => activity.id === id))) return trip;
...
export const migrateTripsForCurrentApp = (trips) => Array.isArray(trips) ? trips.map(swapDomesticActivities) : [];
